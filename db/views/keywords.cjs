// file: C:\_dev\repos\v0-db-api\db\views\keywords.cjs
...
(async () => {
  ...
  try {
    ...
    try {
      await db.collection("keywords").drop();
    ...
    await db.createCollection("keywords", {
      viewOn: "movies",
      pipeline: [
        // Unwind the keywords array to create a document for each keyword
        { $unwind: "$keywords" },
        // Group by keyword id to get unique keywords
        {
          $group: {
            _id: "$keywords.id",
    ...
    console.log("View 'keywords' created successfully.");
  } catch (error) {
